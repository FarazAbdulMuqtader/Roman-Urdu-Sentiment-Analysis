...
# FIX: removed the df.sample(3000, ...) limit and matched the
# stratify=df['label_id'] used in 03_train_model.py so this produces the
# exact same train/test split as training — otherwise the "test" set here
# could include rows the model already saw during training.
train_df, test_df = train_test_split(
    df, test_size=0.2, random_state=42, stratify=df['label_id']
)
print(f"Evaluating on {len(test_df)} held-out rows")
